package com.gystry.recyclerviewapp.recyclerview
...
import com.gystry.recyclerviewapp.R
import com.gystry.libcommon.utils.VIEW_TYPE_ADVERTISING
import com.gystry.libcommon.utils.VIEW_TYPE_STUDENT
import com.gystry.libcommon.utils.VIEW_TYPE_TEACHER
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        rv_view.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                ...
                var findLastVisibleItemPosition = linearLayoutManager.findLastVisibleItemPosition()
                var findFirstVisibleItemPosition = linearLayoutManager.findFirstVisibleItemPosition()

                //记录顶部第一个可见itemview的顶部位置
                val firstVisibleItemView = linearLayoutManager.findViewByPosition(findFirstVisibleItemPosition)
                val top = firstVisibleItemView?.top
                Log.e(TAG, "recyclerTest-onScrollStateChanged: ---newState:${newState}----findLastVisibleItemPosition:${findLastVisibleItemPosition}----findFirstVisibleItemPosition:${findFirstVisibleItemPosition}")

                //可以精确的滑动到指定的位置，根据position和精确的位置x/y轴坐标
                linearLayoutManager.scrollToPositionWithOffset(findFirstVisibleItemPosition, top!!)

                //滑动到指定的position，但是不精确
