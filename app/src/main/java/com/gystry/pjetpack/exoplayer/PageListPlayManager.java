...
public class PageListPlayManager {
    ...
    static {
        ...
        Cache simpleCache = new SimpleCache(application.getCacheDir(), new LeastRecentlyUsedCacheEvictor(1024 * 1024 * 200));
        //把缓存对象cache和负责缓存数据读取，写入的工厂类cacheDataSinkFactory相关联
        ...
        CacheDataSourceFactory cacheDataSourceFactory = new CacheDataSourceFactory(simpleCache,
                dataSourceFactory, new FileDataSourceFactory(), cacheDataSinkFactory, CacheDataSource.FLAG_BLOCK_ON_CACHE, null);
        //MediaSource媒体资源加载的工厂类
        //由它创建的MediaSource能够实现边缓冲边播放的效果
        //如果需要播放hls,m3u8则需要创建DashMediaSource.Factory()
